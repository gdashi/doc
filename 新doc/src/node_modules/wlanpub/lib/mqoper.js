...
MqOper.prototype.sendMsg = function (pkey, msg, callback) {
    ...
    var trueName = pkey.replace(basic.testSuffix, '');
    if (cons.bIsValidService(trueName, true)) {
...
MqOper.prototype.broadcastSendMsg = function (message) {
    ...
    if(broadcastSendExchage)
    {
        ...
        try {
            ...
            console.log((new Date()) + ' [MQHD]Send message to %s success by %s.', broadcast_key, broadcastSendExchage.name);
...
MqOper.prototype.connectReadyForServer = function (recvCallback, serverName) {
    if (undefined === serverName) {
        serverName = cons.serviceName.webserver;
    }
    ...
    var connect = amqp.createConnection(mqOption);
    connect.on('ready', function() {
        console.log((new Date()) + ' [MQHD]Connect to MQ for %s is ready: %s', serverName, JSON.stringify(mqOption));
        ...
        connect.queue(serverName, cons.queueParas.qOption, function (q2) {
            console.log("[MQHD]Queue "+serverName+" for reply is open.");
