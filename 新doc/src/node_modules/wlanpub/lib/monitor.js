...
function Monitor() {
  ...
  // digest RestAPI 相关
  this._keyTailPathSet = ':REST:PATHS';
...
Monitor.prototype.createProcessStats = function (moduleName, redisPara) {
  if (!serName.bIsValidModule(moduleName)) {
    return ;
  }

  // 灰度发布
  moduleName += basic.testEnvSuffix;
...
//    this.redisClient = new redis.Cluster([para]);
  ...
  this.redisClient.select(1);   // 选择Redis数据库序号 1
...
Monitor.prototype.accessStats = function(req, res, next){
  ...
  if (('' != name) && (undefined != name)) {
    ...
    if (isValidService) {
      // 灰度发布
      name += (req.session && ('true' == req.session.bUserTest)) ? basic.testSuffix : '';
...
/**
 * Statistical response information for digest api access
 */
Monitor.prototype.digAccessStats = function(req, res, next){
  var pathname = url.parse(req.url).pathname;
  var reqTimeMS = (new Date()).getTime();

  monitor.redisClient.sadd(monitor._keyPrefix + monitor._keyTailPathSet, pathname);
  monitor._reqCount(pathname, reqTimeMS);

  res.on('finish', function() {
    if ( res.statusCode < 400 ) {
      monitor._resCount(pathname, reqTimeMS);
    } else {
      monitor._resErrCount(pathname, reqTimeMS);
      console.warn('[MONIT] Rest API ' + pathname + ' response failed with status code: ' + res.statusCode);
    }
  });
 ...
 *
 */
Monitor.prototype._saveTimeDiff = function(reqType, mname, sectIndex, timeDiff) {
  ...
  var bucketName = [mname, reqType].join(':') + keyTail;
...
//  var instanceName = [os.hostname(), serviceName, pid].join(':');
  ...
  // 实例基本信息
  this._serviceInfo.name   = serviceName;
  this._serviceInfo.ipv4   = ip;
  this._serviceInfo.status = "run";
  this._serviceInfo.pid    = pid;
  this._serviceInfo.starttime = (new Date()).toLocaleString();
  monitor._getHostname(pid, ip);  // 异步,直到获取到有效值
  ...
  */
  process.on('beforeExit', function() {
    this._serviceInfo.status   = "beforeExit";
    this._serviceInfo.downtime = (new Date()).toLocaleString();

    this._saveBasicInfo();
  }.bind(this));

  process.on('exit', function(code) {
    this._serviceInfo.status   = "exit";
    this._serviceInfo.exitCode = code;
    this._serviceInfo.downtime = (new Date()).toLocaleString();
...
Monitor.prototype._saveBusiness = function(porName) {
  // 灰度发布
  var trueName = porName.replace(basic.testSuffix, '');

  // 将索引值作为分数存入redis setlist中
  this.redisClient.zadd(this._insEntryKey, serName['serviceIdx'][trueName], porName);
};
...
Monitor.prototype._saveHostUsage = function () {
  ...
  var cmd = 'ps aux | grep -E \"^root\\s\+' + monitor._serviceInfo.pid + '.*\"';
 ...
 */
Monitor.prototype._saveBasicInfo = function() {
  var redis       = this.redisClient;
  var serviceInfo = this._serviceInfo;
  ...
  // 存业务名->实例名
  redis.sadd(serviceKey, this._instanceName);
  redis.expire(serviceKey, this._redisKeyTtl);

  // 存主机名->实例名
  redis.sadd(hostnamekey, this._instanceName);
  redis.expire(hostnamekey, this._redisKeyTtl);

  // 存基本信息，确保只存一份
  serviceInfo.updatetime = (new Date()).toLocaleString();
  redis.spop(instanceKey);
  redis.sadd(instanceKey, JSON.stringify(serviceInfo));
  redis.expire(instanceKey, this._redisKeyTtl);
};
...
Monitor.prototype._getHostname = function (pid, ipv4) {
  var redis = monitor.redisClient;
  var subIP = ipv4;
  ...
  (function tryGet() {
    if ('unknown' != monitor._serviceInfo.hostname) {
      monitor._saveBasicInfo();
      return ;
    }
    ...
    redis.get(ip2hostKey, function (error, hostname) {
      if (null == hostname) {
        hostname = 'unknown';
        console.error('[MONIT]Failed to get hostname for ip ' + ip2hostKey + ', Interfaces: ' + JSON.stringify(os.networkInterfaces()));
      }

      monitor._serviceInfo.hostname = hostname;
    });

    setTimeout(tryGet, monitor._usageInterval);
  })();
};
