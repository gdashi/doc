(function ($)
{
        ...
        var opt = {
            ...
            var opt = {
                ...
                colModel: [
                    ...
                    {name: "ReportTime", datatype: "String",width:60},
                    {name: "Channel", datatype: "Integer",width:60},
                    {name: "Rssi", datatype: "Integer",width:50},
    ...
    function aplinkList(){
        ...
        function getClientprobeSuc(data){
            ...
            {
                var date =dataUtil(aClientProbes[i].ReportTime);
                var oTemp = {
                    MacAddress:aClientProbes[i].MacAddress,
                    ApName:aClientProbes[i].SensorName,
                    Channel:aClientProbes[i].Channel,
                    ReportTime:date,
                    Rssi:aClientProbes[i].Rssi||"",
                    BSSID:aClientProbes[i].Bssid
                };
                aClient.push(oTemp);
    ...
    /* 将终端上下线历史模块链接到终端信息页面 */
    function initHealth_History()
    {
        var sUrl = "health.clienthistory";             /* 文件夹名和文件名 */
        var jItem = $('<div class = "col-xs-12 app-colum left-block"></div>'); /* 用jQuery创建一个对象*/
            jItem
                .appendTo($("#terminalinfo_histroy"));                        /* 追加到页面对应ID的位置 */
        Utils.Pages.loadModule(sUrl,null,jItem);                        /* 加载 */
    }
    function _init ()
    {
        initData();
