(function ($)
{
    ...
    function getSSIDInfo()
    {
        function get_SSIDDataSuc(data){
            var aTemplate=[];
            if(data&&data.ssid_list&&data.ssid_list.length>0){
                var wireLess_Status = getRcText("Wireless_Status").split(",");
                for(var i=0;i<data.ssid_list.length;i++)
                {
                    var oTemp={};
                    oTemp.Enable = wireLess_Status[data.ssid_list[i].status];
                    oTemp.Name  = data.ssid_list[i].sp_name;
                    oTemp.SSID = data.ssid_list[i].ssid_name;
