...
(function($) {
    ...
    //    aAllItems = ItemReorder(oDocData.AllItems); /* 文档接口排序 */
    ...
    //         1、后面的接口会替换掉前面的接口；
    //         2、相同标题（类目）的会排在一起 */
    //     function ItemReorder(aItems)
    //     {
    //         for(var i = 0; i < aItems.length; i++)
    //         {
    //             var sItemName = aItems[i].ItemName;
    //             var sSubItem = aItems[i].ItemName + '_' + aItems[i].SubItem;

    //             oSubItem[sSubItem] = aItems[i];
    ...
    function initForm()
    {
        $(".modal").draggable();    //模态框可以拖动
        $("#docListPage").removeClass("hide");

        /* 绑定页面按钮点击事件 */
        $('#docLeftBar [name="home"]').click(showDocList);
        $('#docLeftBar [name="add"]').click(addDoc);
        $('#docLeftBar [name="edit"]').click(editDoc);
        $('#docLeftBar [name="upload"]').click(uploadDoc);
        ...
        {
            $(".container-fluid").addClass("hide");
            $("#addDocPage .header").text(getRcText("ADDDOC"));
            $("#addDocPage").removeClass("hide");
        }
        function editDoc()
        {
            $(".container-fluid").addClass("hide");
            $("#addDocPage .header").text(getRcText("EDITDOC"));
            $("#addDocPage").removeClass("hide");
        }
        function uploadDoc()
        {
            $(".container-fluid").addClass("hide");
            $("#uploadDocPage").removeClass("hide");
        }
    }
